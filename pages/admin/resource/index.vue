  ...
  <div>
    <z-panel title="文章资源管理">
      <template v-slot:headerRight>
        <btn @click="handleAddResource" theme="gradient">新增资源</btn>
      </template>
    ...
    <modal v-show="isShowResourceModal" @close="handleHideResourceModal">
      ...
      <div slot="footer">
        <btn :loading="isAddLoading || isEditLoading" @click="handleSubmitResource" theme="gradient" long>{{ editMode === 'edit' ? '确认修改' : '确认添加' }}</btn>
      </div>
    </modal>
  ...
  methods: {
    ...
    handleCheckFormData() {
      if (!this.formData.resourceTypeId) {
        this.$toast.error('请选择资源类别', { icon: 'iconfont toasted-icon iconerror' })
        return false
      } else if (!this.formData.name) {
        this.$toast.error('请填写资源名称', { icon: 'iconfont toasted-icon iconerror' })
        return false
      }
      return true
    },
